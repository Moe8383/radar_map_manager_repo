...
* **坐标实体命名规范**：
    ...
    * 2D/3D 雷达：sensor.[radar_name]_target_?_x 、 sensor.[radar_name]_target_y 、 sensor.[radar_name]_target_z, 例如：sensor.rd_ld6004_target_1_x, sensor.rd_ld6004_target_1_y, sensor.rd_ld6004_target_1_z
...
#### 1. 添加/删除雷达
1.1 添加：点击 + 号，输入在HA中的雷达名称，如雷达的坐标为 sensor.rd_ld2450_target_1_x，则输入雷达名称：rd_ld2450
1.2 删除：选中已添加的雷达，点击 - 号，删除该雷达，慎重操作。

#### 2. 雷达设置

![LAYOUT](images/layout_2.png)

2.1 雷达定位：拖动雷达到实际摆放的位置，拖动雷达把手选择角度。位置和角度可以通过面板的X/Y/Rot进行精确调整。
2.2 调整比例：站在雷达监测范围内（建议尽量远离中线，并尝试多个位置提高精准度），建议综合采用以下几种方法，使雷达目标与户型图中显示的目标吻合：
2.2.1 通过调整面板中的 ScX 和 ScY ，调整 X Y 坐标的显示比例
2.2.2 点击 Ax Ay 会根据背景图比例自动调整（供参考）
2.2.3 点击 Freeze 按钮，会锁定雷达识别的第1个目标，手动拖动该目标至你所处于的户型图位置，会自动计算缩放比例
2.3 安装方式：勾选面板底部的 Ceiling , 切换“侧装/顶装”安装方式
2.4 镜像模式：勾选面板底部的 Mirror ， 反转雷达 X 轴
2.5 3D矫正： 勾选面板底部的 3D，输入雷达安装高度（单位：m），将进行目标3D矫正。如果雷达安装高度在建议范围内，可以不选择3D矫正。
2.6 UNDO：撤销上一步操作
...
3.1 通过面板或底图选中某个雷达，点击面板 Monitor 按钮，进入该雷达的监测区域设置
3.2 新增区域：点击 ADD NEW 按钮，进入添加区域模式，通过选定多边形端点确定形状，并为区域命名，点击 FINISH 确认保存。
3.3 调整区域：选中某区域，通过拖动端点，可以灵活调整区域形状，双击某个端点可以删除该端点。
3.4 删除区域：选中某区域，点击 DEL 可以删除选中的区域；点击 CLR ALL 删除该雷达下的所有Monitor区域（审慎操作！！！）
3.5 点击 DONE 退出Monitor区域编辑。
...
#### 1. Detect Trigger 区域
1.1 编辑操作与 Monitor 区域相同
1.2 支持设置目标进入延时，在 Dly 中输入延时时间（单位：s），用于避免雷达目标异常导致误报
1.3 用于自动化触发，设置该区域后，会自动创建 “存在” “人数” 2个实体

#### 2. Detect Exclude 区域
2.1 编辑操作与 Monitor 区域相同
2.2 融合目标落入该区域内，则不显示、不触发，用于排除风扇、空调等干扰
...
1.1 Updata: 雷达目标刷新频率
1.2 Merge: 雷达目标融合距离，在该距离以内的不同雷达的目标进行融合
1.3 Tgr H: 目标质心高度，用于3D矫正
1.4 Color: 融合目标自定义颜色
1.5 Backup: 备份当前配置并导出文件
1.6 Restore: 导入文件恢复配置
